...
**The non-random assembly of indirect interactions in plant-pollinator networks**
...
**Figure S1** Map with the plant-pollinator study locations.

**Figure S2** Over and under-estimated motif frequencies.

**Figure S3** Over and under-estimated groups on motif positions.

**Figure S4** Plant trait composition of functional groups.

**Figure S5** Dendrogram with plant functional groups.

**Figure S6** Association between the percentile and specificity of the different plant and floral visitor groups on motif positions.

**Figure S7** Association between the percentile and number of indirect interactions of the different plant and floral visitor groups on motif positions.

**Figure S8** Frequency of floral visitor groups and number of indirect interactions per motif positions.

**Figure S9** Frequency of plant groups and number of indirect interactions per motif positions.
...
Trait1 <- c("Lifepan", "Life form", "Flower shape", "Flower symmetry", "Compatibility system", "Breeding system", "", "")

T.cat <- linebreak(c("Short-lived \n Perennial", "Herb \n Shrub \n Tree", "Brush \n Campanulate \n Capitulum \n Open \n Papilionaceous \n Tube",  "Actinomorphic \n Zygomorphic",  "Self-incomp. \n Partially self-comp. \n Self-comp.", "Hermaphrodite \n Monoecious \n Dioecious","", ""),align = c("l"))

Type <- c("Vegetative", "Floral", "Floral", "Floral", "Floral", "Floral", "Floral","Reproductive")

Type_1 <- c("Vegetative", "Vegetative", "Floral", "Floral", "Reproductive", "Reproductive", "", "")
...
colors <- c("#FFA405", "#0075DC", "#993F00", "#4C005C", "#5EF1F2", 
        "#005C31", "#2BCE48", "black", "lightblue", "#94FFB5", 
        "#8F7C00", "#9DCC00", "#C20088", "#003380", "gold1", 
        "#FFA8BB", "#426600", "#FF0010", "#191919")
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE,fig.height=7, fig.width=12, fig.cap="**Figure S4.** Association between the percentile (frequency comparison between empirical and simulated networks) and specialization of the different plant and floral visitor groups on the distinct motif positions for 1000 random sets that contain the 60% of the whole set of networks each. Black lines represent the mean percentile values of random samples and the continuous red line indicates the mean percentile values for the whole set of networks. Grey areas show the 95% confidence intervals for random samples, whereas those for the whole set of networks are displayed by using red dashed lines. Note that percentiles above 0.75 indicate over-representation and those below 0.25 under-representation in the comparison between empirical and simulated networks."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE,fig.height=7, fig.width=12, fig.cap="**Figure S5.** Association between the percentile (frequency comparison between empirical and simulated networks) and number of indirect interactions of the different plant and floral visitor groups on the distinct motif positions for 1000 random sets that contain the 60% of the whole set of networks each. Black lines represent the mean percentile values of random samples and the continuous red line indicates the mean percentile values for the whole set of networks. Grey areas show the 95% confidence intervals for random samples, whereas those for the whole set of networks are displayed by using red dashed lines. Note that percentiles above 0.75 indicate over-representation and those below 0.25 under-representation in the comparison between empirical and simulated networks."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE,fig.height=7, fig.width=12, fig.cap="**Figure S6.** Plant functional group composition separated in qualitative and quantitative variables. Panel A) shows the percentage of the different categories within trait represented with different colours for each functional group. Plot B) shows the radar plot of the different quantitative variables standardise on the same scale also coloured with the same patterns of colours as qualitative variables per cluster or functional group. Group 1 corresponds to short-lived selfers; group 2 to small outcrossing perennials; group 3 to self-incompatible perennials with large flowers; group 4 to tall plants with small unisexual flowers; and, group 5 to short-lived outcrossers with long zygomorphic flowers."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, results='hide', warning=FALSE, fig.height=7, fig.width=13, fig.cap="**Figure S7.** Hierarchical clustering dendrogram with the branches coloured by the optimal number of clusters (5). The labels of the subgroup of species (N = 524) used in this study are coloured in black in order to show the evenness of the distribution of the species across clusters. The rest of species labels are omitted for visualization purposes (N = 982)."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE,  warning=FALSE,fig.align="center", fig.width=14,fig.height=7, out.width='100%',  fig.cap="\\textbf{Figure S8.} Association between the percentile (see main text) and specialization of the different plant and floral visitor groups on motif positions. Here we consider that percentiles above 0.75 indicate over-representation and percentiles below 0.25 under-representation in the comparison between empirical and simulated networks."}
...
pollinators_sp_plot <- visreg(model_freq_specif_ind, "Specificity",scale="response", by="Group",line.par = list(col = 'black'),points=list(size=0),
                         strip.names=c("Bee",
                                       "Coleoptera",
                                       "Lepidoptera",
                                       "Non-bee-Hymenoptera",
  ...
  xlab("Specialization") + ylab("Percentile")+
  scale_x_continuous(breaks = seq(0, 1, by = 0.5))+ scale_fill_grey(start=0.5, end=0.8) +
   theme(strip.text.x = element_text(size = 12),plot.title = element_text(hjust = 0.5, size = 23))+
    ggtitle("Floral visitors")+ylim(-0.15,1)+geom_point(data=pollinator_freq_position_s_ind, aes(Specificity,Percentil), alpha=0.1)



plants_sp_plot <- visreg(model_freq_specif_ind_plant, "Specificity",line.par = list(col = 'black'), by="Group",scale="response",points=list(size=0),
  ...
  scale_x_continuous(breaks = seq(0, 1, by = 0.5))+
   theme(strip.text.x = element_text(size = 12),plot.title = element_text(hjust = 0.5, size = 23))+
  ggtitle("Plants")+ylim(-0.15,1)+geom_point(data=plant_freq_position_s_ind, aes(Specificity,Percentil), alpha=0.1)


library(patchwork)
pollinators_sp_plot/plants_sp_plot & theme(axis.text=element_text(size=10),
        axis.title=element_text(size=16)) & plot_annotation(tag_levels = c('A'),  tag_suffix = ')') &  theme(plot.tag = element_text(size = 22))
...
```{r, echo=FALSE, message=FALSE, cache=FALSE,  warning=FALSE,fig.align="center", fig.width=14,fig.height=7, out.width='100%', fig.cap="\\textbf{Figure S9.} Association between the percentile (see main text) and number of indirect interactions of the different plant and floral visitor groups on motif positions. Here we consider that percentiles above 0.75 indicate over-representation and percentiles below 0.25 under-representation in the comparison between empirical and simulated networks."}
...
pollinators_sp_plot <- visreg(model_freq_specif_ind, "Indirect_interactions", scale="response", by="Group",line.par = list(col = 'black'),
                         strip.names=c("Bee",
                                       "Coleoptera",
                                       "Lepidoptera",
                                       "Non-bee-Hymenoptera",
                                       "Non-syrphids-diptera",
                                       "Syrphids"),gg = TRUE) + theme_bw() +
  xlab("Number of indirect interactions") + ylab("Percentile")+
 # scale_x_continuous(breaks = seq(0, 1, by = 0.5))+
  scale_fill_grey(start=0.5, end=0.8) +
   theme(strip.text.x = element_text(size = 12),plot.title = element_text(hjust = 0.5, size = 23))+
    ggtitle("Floral visitors") +ylim(-0.15,1)+geom_point(data=pollinator_freq_position_s_ind, aes(Indirect_interactions,Percentil), alpha=0.5)



plants_sp_plot <- visreg(model_freq_specif_ind_plant, "Indirect_interactions",line.par = list(col = 'black'), by="Group",scale="response",
       strip.names=c("Selfing herbs",
                     "Small outcrossing perennials",
                     "Self-incompatible perennials\nwith large flowers",
                     "Tall plants with small\nunisexual flowers",
  ...
  #scale_x_continuous(breaks = seq(0, 1, by = 0.5))+
   theme(strip.text.x = element_text(size = 12),plot.title = element_text(hjust = 0.5, size = 23))+
  ggtitle("Plants")+ylim(-0.15,1)+geom_point(data=plant_freq_position_s_ind, aes(Indirect_interactions,Percentil), alpha=0.5)
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, warning=FALSE,out.width="90%",fig.align="center", fig.width=10,fig.height=12, fig.cap="\\textbf{Figure S10.} Frequency of the different floral visitor groups on the different motif positions aggregated by the number of indirect interactions for the different motif positions."}
...
```{r, echo=FALSE, message=FALSE, cache=FALSE, warning=FALSE,out.width="90%",fig.align="center", fig.width=10,fig.height=12, fig.cap="\\textbf{Figure S11.} Frequency of the different plant groups on the different motif positions aggregated by the number of indirect interactions for the different motif positions."}
