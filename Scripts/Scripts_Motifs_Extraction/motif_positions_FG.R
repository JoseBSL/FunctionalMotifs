...
for (i in 1:length(motif_link_files)){
  ...
  motif_links_i <- read_csv(file_i) %>% 
    gather("Type","FG",-c(Network_id, Motif_pattern_id,
                               Nodes, Motif_number)) %>%
    arrange(Motif_pattern_id, Nodes,Motif_number) %>%
    group_by(Network_id, Motif_pattern_id,
             Nodes,Motif_number, Type,FG) %>% count() %>%
    rename(Degree = n,Node_id = FG)
  
  # Classify nodes: Plant - Pollinator
  motif_links_i$Type[motif_links_i$Type == "Plant_functional_groups"] <- "Plant"
  motif_links_i$Type[motif_links_i$Type == "Pollinator_functional_group"] <- "Pollinator"
  ...
  pollinator_FG <- networks_raw %>% filter(Network_id == network_i) %>% 
    dplyr::select(Pollinator_functional_group) %>% unique() %>%
    rename(Node_id = Pollinator_functional_group) %>%
    mutate(Functional_group = Node_id)
  
  plant_FG <- networks_raw %>% filter(Network_id == network_i) %>% 
    dplyr::select(Plant_functional_groups) %>% unique() %>%
    rename(Node_id = Plant_functional_groups) %>%
    mutate(Functional_group = Node_id)
  
  plant_FG$Node_id <- as.character(plant_FG$Node_id)
...
read_csv("Data/Csv/Motifs links for FG/Motifs_positions_15_2_peralta_2006_mendoza_site_2.csv") %>%
  filter(Node_id=="Bee") %>% 
