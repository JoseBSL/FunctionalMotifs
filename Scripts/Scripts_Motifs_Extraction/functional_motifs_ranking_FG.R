...
networks_raw <- read_csv("Data/Csv/data_for_motifs_analysis_1.csv")

# Load raw data with plant and pollinators
frequency_raw <- read_csv("Data/Csv/network_frequency_motifs_FG.csv")

# List the files with summary information about the amount of functional motifs
folder_motif_data <- "Data/Csv/Motifs links for FG"
...
for (i in 1:length(motif_summary_files)){
  ...
  species_i <- networks_raw %>%
    ...
    dplyr::select(Pollinator_functional_group,Plant_functional_groups)
  
  summary_i$Total_plants <- unique(species_i$Plant_functional_groups) %>% length()
  summary_i$Total_pollinators <- unique(species_i$Pollinator_functional_group) %>% length()
...
write_csv(all_networks_scores,"Data/Csv/functional_motifs_percentages_FG.csv")
...
for(i in 1:nrow(functional_motif_results)){
  ...
  functional_motif_results$Mean_normalize_sum[i] <- mean(aux_i$normalize_sum,na.rm = T)
  functional_motif_results$Mean_normalize_sizeclass[i] <- mean(aux_i$normalize_sizeclass,na.rm = T)
  functional_motif_results$Mean_normalize_nodesets[i] <- mean(aux_i$normalize_nodesets,na.rm = T)
  functional_motif_results$Sd_normalize_sum[i] <- sd(aux_i$normalize_sum,na.rm = T)
  functional_motif_results$Sd_normalize_sizeclass[i] <- sd(aux_i$normalize_sizeclass,na.rm = T)
  functional_motif_results$Sd_normalize_nodesets[i] <- sd(aux_i$normalize_nodesets,na.rm = T)
}

# Save percentages of functional motifs

write_csv(functional_motif_results,"Data/Csv/functional_motif_mean_values_FG.csv")
