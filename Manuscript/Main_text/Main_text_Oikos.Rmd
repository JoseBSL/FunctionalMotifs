...
Overall, generalist network motifs for floral visitors were consistently over-represented (e.g. fan-type motifs; **Figure 2**). For instance, when we compare motifs with equal structure for both trophic levels, we found that floral visitors had always over-representation in the positions that implied the highest number of direct links. Despite this is in good agreement with the current view of the generalist tendency of pollinators [@waser1996; @olesen2002; @ollerton2017], we found that this over-representation of the generalist network motifs was only driven by the taxonomic group of bees (**Figure S2**) while the rest of the taxonomic groups tended to be under-represented in these motif that involved generalism and over-represented in motif positions with specialist roles. This large effect of bees on the overall patterns is no surprise, as bees were the group with largest number of interaction records in the whole set of networks as also recorded for other broad scale plant-pollinator network studies [@carvalheiro2014; @ollerton2017]. Interestingly, bees also were the group with lower number of indirect interactions while the rest of floral visitor groups showed clearly higher number of indirect interactions indicating that these groups may experience higher levels of competitive interactions due to greater overlap of floral resources [@thomson2020]. Unlike floral visitors, plant functional groups showed little differences on its appearance in different motif positions, and thus similar probability of appearing on all of them. However, the groups that involved higher levels of outcrossing showed a tendency to be more over-represented and probable on the different motif positions. Remarkably, the group of tall plants with unisexual flowers, the one that is expected to have the highest dependence of pollinators on reproduction [@rodger2021], showed the lowest level of indirect interactions but the highest frequency of records for all groups which involves a high number of direct interactions that are likely to minimise negative impacts of pollinator sharing (e.g., heterospecific pollen arrival).
