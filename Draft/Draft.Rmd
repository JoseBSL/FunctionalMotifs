...
First, plant species were grouped into the optimal number of functional groups that summarized the main plant reproductive strategies. This was done with the help of hierarchical clustering by using the trait dataset collated in Lanuza et al., (unpublished) that comprised 1,506 plant species and contained the same 60 plant-pollinator networks used in this study plus some non-weighted and weighted metawebs (see Table S1 Lanuza et al., unpublished). This dataset consisted on 8 floral, 4 reproductive and 3 vegetative traits excluding the the traits with high percentage of missing values (over 30%; Table S2). We opted to calculate the plant functional groups on this larger set of species because of the higher accuracy when delimiting the functional groups with that many variables and species @dolnicar2014. For this, we calculated the distance of the different functional traits with the function _gowdis_ from the package _FD_ version 1.0-12 [@laliberte2014] with the method _ward.D2_ used for non-squared distances [@murtagh2014]. All numerical traits were previously scaled. Finally, we conducted hierarchical clustering with the function _hclust_ from the _R_ stats package version 4.0.5 and calculated the optimal number of clusters with the function _kgs_ from the package _maptree_ version 1.4-7 [@white2009].
