...
The interaction between plants and pollinators can be studied at different scales, from species level interactions (micro-scale) to the full network structure (macro-scale). Research of the network structure have proven common invariant structural properties across them including nestedness [@jordano2003], the imbalance between plants and pollinators with approximately four times more pollinator species [@vazquez2009] and a degree distribution that decays as a power law [@jordano2003]. In addition, phenological overlap, morphological matching and species abundances have been shown to be determinant for the understanding of plant-pollinator interactions and network structure [@stang2006; @peralta2020]. However, condensing to species level and the holistic view of the full network involves missing relevant information for the understanding of ecological processes [@simmons2019]. 

Indirect interactions (i.e., motifs), commonly referred as the building blocks that depict subsets of interactions of the network [@milo2002], are a widespread phenomenon in ecological communities [@strauss1991]. For instance, plant species can facilitate or compete for pollinator's visitation. Despite their relevance in ecological communities, plant-pollinator research fails to capture indirect interactions with the traditional analytical tools that condense the information in single indices (e.g., netedness or interaction frequency). Nonetheless, the emerging framework of network motifs in plant-pollinator research allows to consider both direct and indirect interactions [@simmons2019]. The study of @simmons2020 have shown that the different processes that govern species interactions (e.g., species abundances versus trait-matching) can lead to different patterns of indirect interactions. However, we still ignore what are the general patterns of indirect interactions in plant-pollinator networks.

Linking the structural porperties of the meso-scale with the species ecology can help the understanding of ecological processes. For instance, different motifs can have different ecological meanings [@simmons2019] and the position within a motif can determine the species functional role [@stouffer2012; @baker2015]. However, motifs are abstract representations often decoupled from species ecology. Remarkably, recent empirical findings indicate that the meso-scale is the best descriptor of plant reproductive success [@allen2021]. Further, although some studies have evaluated plant reproductive strategies in plant-pollinator networks [tur2013; @lazaro2020], they are often overlooked [@devaux2014]. Hence, exploring how the main reproductive groups integrate with the emergent motif framework can shed light of key aspects of ecosystem functioning.

Here, we used 60 plant-pollinator networks from 18 different studies and grouped the different plant and pollinator species into functional groups that summarized the main reproductive strategies. Plants were grouped based on a comprehensive dataset that included floral, reproductive and vegetative traits compiled in Lanuza et al., (unpublished) on a larger set of plant-pollinator networks. Pollinators were grouped into the main taxonomical groups that differed in life form and behaviour. Then, we explored the frequency of motifs up to five nodes in these set of plant-pollinator networks. Finally, we explore if the different plants and pollinators functional groups are over- or under-represented by comparing their frequencies with simulated networks.
...
```{r, echo=FALSE, message=FALSE, cache=FALSE,  warning=FALSE, fig.height=8,fig.width=8, fig.cap="Figure 3. . Comparison of the motif frequencies between empirical and simulated networks collapsing the networks nodes by functional plant and pollinator functional groups. Mean percentiles close to 0 and 1 indicate under- and over- representation respectively."}
...
Heatmap indicating under- and over- representation of pollinator and plant functional groups in the different motif positions. The different motif positions are dividied by the average path length clasification by @simmons2020
