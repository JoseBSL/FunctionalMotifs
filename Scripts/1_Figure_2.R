...
plot_motif_positions <- function(motif_number){
  ...
  ggplot(data=d_nodes, aes(x, y)) +
    ...
    geom_segment(data = link_df, aes(x=xdown, y=ydown, xend = xup, yend = yup), size = 2, color = "grey20")+
    geom_point(aes(color = as.factor(label)),size=32, shape=21,color="black",fill="black")+
   # geom_text(aes(x, y , label = label),size=4)+
