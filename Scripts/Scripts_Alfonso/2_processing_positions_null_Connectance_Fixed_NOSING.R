...
folder_motif_data <- "Data/Csv/Motifs_positions_and_null_models_NOSING"
...
for(i in 1:length(motif_link_files)){
  ...
  new_file_i <- paste0(folder_motif_data, "/Position_quantiles_Conn_Fixed_NOSING_", network_i,".csv")
...
####################
# Save every Position_quantiles_Conn_Fixed_NOSING in a single file

# List the files with information about the positions
folder_motif_data <- "Data/Csv/Motifs_positions_and_null_models_NOSING"

motif_files <- list.files(folder_motif_data) 
quant_link_files <-  motif_files[grep("Position_quantiles_Conn_Fixed_NOSING_",motif_files)]
...
final_file_i <- paste0(folder_motif_data, "/GF_positions_frequency_percentile_NOSING.csv")
