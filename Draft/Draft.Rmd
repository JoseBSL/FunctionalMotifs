...
```{r, echo=FALSE, message=FALSE, cache=FALSE,  warning=FALSE, fig.cap="Figure 2. Comparison of the motif frequencies between empirical and simulated networks. Average percentages of motifs close to 0 and 100 indicate under- and over- representation in empirical networks. The different motifs are coloured by the mean path length as done in @simmons2020.", fig.width=8,fig.height=5}
library(lme4)
library(nlme)
library(tidyverse)
...
ggplot(NULL) + 
  ...
  theme_bw() + scale_color_manual(labels = c('Fan' = expression(paste("Fan (", bar(x), " = 1.48)")), 'Medium-weak' = expression(paste("Asymmetric complete (", bar(x), " = 1.60)")),
    'Strong' = expression(paste("Complete (", bar(x), " = 1.38)")), "Weak" =expression(paste("Core-peripheral (", bar(x), " = 1.85)"))), name = "Path length classification",values = c("deepskyblue3","maroon","darkgoldenrod2","grey25")) +
