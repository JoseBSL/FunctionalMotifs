...
```{r, echo=FALSE, message=FALSE, cache=FALSE,  warning=FALSE, fig.width=9,fig.height=5, fig.cap="**Figure 2.** Comparison of motif frequencies between empirical and simulated networks. This is shown with the mean percentage of motif frequencies in empirical networks that were over the motif frequencies of the simulated ones (percentiles). This was done by network (coloured dots) and then averaged for all networks (black dots with error bars that correspond to the standard deviation). The different motifs are coloured by the four categories of path length determined in Simmons et al. (2020)."}
library(lme4)
library(nlme)
library(tidyverse)
...
ggplot(NULL) + 
  ...
  geom_point(data = motif_means,aes(y = as.factor(motif), x=100*mean), 
                size=2)+
  labs(y="Two to five node motifs", x = "Percentile",colour="Path length classification")+
  theme_bw() + scale_color_manual(labels = c('Strong' = expression(paste("Complete (", bar(x), " = 1.38)")),'Fan' = expression(paste("Fan (", bar(x), " = 1.48)")), 'Medium-weak' = expression(paste("Asymmetric complete (", bar(x), " = 1.60)")), "Weak" =expression(paste("Core-peripheral (", bar(x), " = 1.85)"))), name = "Path length classification",values = c("grey36","lightblue3","navajowhite2","darkorange4"),guide = guide_legend(title.position = "top",ncol = 1)) +
