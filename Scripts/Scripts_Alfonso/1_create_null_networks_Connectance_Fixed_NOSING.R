...
for (i.network in 1:length(list_Network_id)){
  ...
  networks_i <- networks_i %>% 
    ...
    count(wt = Interaction) %>% rename(Interaction = n) %>% filter(Interaction > int.threshold)
  ...
  folder_motifs_pos <- "Data/Csv/Motifs_positions_and_null_models_NOSING"
  file_motifs_pos <- paste0(folder_motifs_pos,"/Binary_Mat_Sp_Motifs_positions_null_Conn_FiX_NOSING_",list_Network_id[i.network],".csv") 
  write_csv(node_positions_i,file_motifs_pos)
  
  folder_motifs_fre <- "Data/Csv/Motifs_frequencies_and_null_models_NOSING"
  file_motifs_fre <- paste0(folder_motifs_fre,"/Binary_Mat_Sp_Motifs_frequencies_null_Conn_FiX_NOSING_",list_Network_id[i.network],".csv") 
