...
# 1) It generates a file with the motif positions of those nodes in 
# "Data/Csv/Motifs" files (which contain the links of each motif that are
# present in a given network).
# The resulting full info is also storaged at "Data/Csv/Motifs": "Motifs_positions_" files
# 
# 2) A summary with the counts for each functional motif is storaged at "Data/Csv/Motifs":
# "Summary_functional_motifs_" files
#
# 3) It calculates the functional motifs observed probabilities, and storage them
# at "Data/Csv/node_motifs_observed_probability.csv"

# Load libraries
library(tidyverse)

# Load raw data with functional groups
networks_raw <- read_csv("Data/Csv/data_for_motifs_analysis_1.csv") %>% select(-X1)
...
for (i in 1:length(motif_link_files)){
  ...
  # We add motif_pattern_ID to Motif_functional_ID
  
  motif_links_i_FG$Motif_functional_ID <- paste(motif_links_i_FG$Motif_pattern_id,
                                                motif_links_i_FG$Motif_functional_ID,
                                                sep = "_")
...
for (i in 1:length(motif_summary_files)){
  ...
  motif_summary_i <- read_csv(file_i) %>%
    rename(motif = Motif_pattern_id,
           motif_functional_ID = Motif_functional_ID)
  
  motifs_observed_probability <- bind_rows(motifs_observed_probability,motif_summary_i)
  
}

motifs_observed_probability <- motifs_observed_probability %>% 
  group_by(motif,motif_functional_ID) %>%
  count(wt = Counts) %>% rename(counts_observed = n) %>% arrange(desc(counts_observed))

total_observations <- sum(motifs_observed_probability$counts_observed)

motifs_observed_probability$motif_observed_probability <- 
  motifs_observed_probability$counts_observed / total_observations

write_csv(motifs_observed_probability,"Data/Csv/motifs_observed_probability.csv")
